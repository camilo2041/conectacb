...
- **Chatbots: usen `respuesta`.** El texto de `/asistente` ya trae la tarifa en formato colombiano (`$3.550`), el código de cada ruta del SITP y los paraderos de subida y bajada ("toma el SITP 6-3 (Sierra Morena - Portal Tunal) en CED. Ciudad Bolívar y bájate en Portal Tunal…"). Si arman su propio mensaje con `ruta.tramos`, los campos son `codigo`, `parada_desde`, `parada_hasta`, `espera_seg` y `tarifa_cop`; un viaje puede tener una, dos o más líneas.
